...
// Función para convertir un string en un enumerado
...
bool avanzaCarril(tCarretera carretera, int i, bool DEBUG) {
    ...
    if (carretera[i].coche.tiempoParado == 0) {
        ...
        if (haLlegado(carretera[i].coche.pos)) {
            ...
            // Si el coche está en una sorpresa, dibujar de nuevo la carretera
...
void avanzaCarriles(tCarretera carretera, tClasificacion& clasificacion, bool DEBUG) {
    ...
    for (int i = 0; i < NUM_CARRILES; i++) {
        ...
        // El !Llegado debería estar en avanzaCarril, pero si lo ponemos aqui nos ahorramos una llamada a la función avanzaCarril
        if (!haLlegado(carretera[i].coche.pos)) {
...
// Operador de inserción de la clasificación en un flujo de salida
...
// Función para solicitar el id de la carretera
...
tClasificacion simulaCarrera(tCarretera carretera) {
    ...
    // Crear un objeto de tipo tClasificacion para guardar la clasificación
...
void guardaListaClasificación(const tListaClasificacion& listaC) {
...
int main() {
    ...
    do {// Mientras el usuario quiera hacer otra simulación, simular la carrera
        ...
        cargaCarretera(carretera);  // función booleana que devuelve true si se ha podido cargar la carretera (no se utiliza)
        dibujaCarretera(carretera);
